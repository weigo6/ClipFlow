...
ImageToMarkdownRunner::ImageToMarkdownRunner(OpenAIChatClient* client, QObject* parent)
    : QObject(parent)
    , m_client(client)
{
}

bool ImageToMarkdownRunner::isRunning() const
{
    return m_running;
}

void ImageToMarkdownRunner::cancel()
{
    if (!m_running) {
        return;
    }
...
void ImageToMarkdownRunner::run(const QImage& image, const AppConfig& cfg)
{
    if (!m_client) {
    ...
    QJsonArray content;
    content.append(QJsonObject{{QStringLiteral("type"), QStringLiteral("text")}, {QStringLiteral("text"), c.prompt}});
    ...
    m_reply = m_client->postChatCompletions(
        ...
        [this](QString content) {
            ...
            emit success(cleanMarkdown(std::move(content)));
        },
        [this](QString error) {
            m_running = false;
            m_reply = nullptr;
